...
```rust
fn engine_function<T>(obj: impl AsObjectArg<T>) { }
```

**After (0.4):**

```rust
fn engine_function<T>(obj: impl AsArg<Gd<T>>) { }
```
...
```rust
let callable = Callable::from_fn(|args| 42);

let callable = Callable::from_fn(|args| {
    do_sth(args);
});
```
...
- Struct `ClassName` -> [`ClassId`][api-classid], which represents the lightweight idea of IDs better.
...
[api-classid]: https://godot-rust.github.io/docs/gdext/master/godot/obj/struct.ClassId.html
...
[#1320]: `#[export(range = ...)]` attributed traditionally required float literals, even for integer fields. The values were also not validated
...
```rust
use godot::builtin::SignedRange;

let slice = array.slice(SignedRange::new(start, end));
```
